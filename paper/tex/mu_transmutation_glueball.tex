...
\title{Dimensional transmutation in a hidden SU(3) sector: the ghost
dark-energy scale μ ≈ 28.5 MeV and glueball dark matter}
...
\begin{abstract}
The QCD-ghost (Veneziano contact-term) dark-energy law
\(\rho = \mu^3 H\) requires a mass scale \(\mu \approx 28.5\) MeV. We
ask whether \(\mu\) is the confinement scale of a hidden \(SU(3)\)
sector fixed by dimensional transmutation. A pure hidden Yang--Mills
sector confining at \(\mu\) requires a Planck-scale coupling
\(\alpha_h = 1/83.2\): log-natural, replacing a tuning of one part in
\(10^{60}\), though the exponential map retains a sensitivity of 47.5. A
sharper variant, \emph{mirror unification} --- hidden and visible
couplings coinciding at the Planck scale, with six flavours ---
introduces \textbf{zero new continuous parameters} and lands in the
right decade: the loop and scheme band is 28--105 MeV at two loops,
widening to 28--133 MeV through four-loop running. The same sector's
glueballs are viable dark matter through \(3\to2\) cannibal freeze-out
--- a mass miracle, not an abundance one --- with a velocity-independent
self-interaction. Cluster bounds and a conditional Little-Red-Dot
seeding argument converge, as constraints, near 52 MeV. That scale and
the 28.5 MeV amplitude scale are one sector, related by the contact-term
coefficient \(6^{1/3}\): their cubed ratio of 6 is a predicted,
lattice-testable topological factor rather than an independent tuning,
to the accuracy of the pure-glue relations it rests on. Three limits are
explicit. The dark-energy law is inherited from a contact term that
remains contested and unproven, and is the single load-bearing
conjecture. Near the conformal-window edge the perturbative band is
irreducibly order-unity wide, so separating the coefficient choices is a
lattice question. Every claim is tagged derived, matched, or
conjectured.
\end{abstract}

\begin{keyword}
dark matter \sep hidden sector \sep dimensional transmutation \sep glueball dark
matter \sep self-interacting dark matter \sep dark energy
\end{keyword}
...
The QCD-ghost, or Veneziano contact-term, construction gives a
dark-energy density linear in the Hubble rate,
\(\rho_{\rm DE} \propto \Lambda^3 H\)
\citep{Urban:2009vy, Urban:2009yg, Ohta:2010in, Cai:2010uf}. Taken with
the visible-sector scale \(\Lambda_{\rm QCD}\) it overshoots the
observed dark-energy density, and the construction is usually left
there. Substituting a \emph{hidden} confinement scale instead,

\begin{equation}\rho_X = \mu^3 H, \qquad \mu \approx 28.5\ \text{MeV},\end{equation}

reproduces the observed amplitude --- but only by trading one
unexplained number for another, since \(28.5\) MeV put in by hand is a
vacuum energy tuned to one part in \(\sim 10^{60}\) against the Planck
scale. The question this paper asks is whether that scale can instead be
\emph{generated}.

Dimensional transmutation \citep{Coleman:1973jx} is the natural
candidate, and the only known mechanism converting an \(O(1)\)
...
We assess the proposal that \(\mu\) is the confinement scale
\(\Lambda_h\) of a hidden \(SU(3)\) gauge sector, in two versions of
increasing economy --- a postulated coupling, then a
\emph{mirror-unified} coupling with no new continuous input --- together
with the mechanism that would convert the \emph{scale} into the
\emph{law} (the Urban--Zhitnitsky/Ohta topological vacuum energy), and
the observation that the same sector's glueballs are a viable
dark-matter candidate with distinctive, velocity-independent
self-interactions.

The law itself is not new: it is the established contact-term result
with the single substitution
\(\Lambda_{\rm QCD} \to \Lambda_{\rm hidden}\). Nor is glueball dark
matter new
\citep{Boddy:2014yra, Soni:2016gzf, Halverson:2016nfq, Forestell:2017wov}.
The contribution here is (i) fixing \(\mu\) by dimensional transmutation
of a hidden \(SU(3)\), and (ii) tying that same sector to glueball dark
matter, so that one confinement scale carries both roles. Neither
ingredient is new on its own; what is new is that they share a scale.
Notably the same \(H\)-linear form is reached from an entirely different
route --- a volume-law horizon entropy gives \(\rho \propto H\) in
entropic cosmology \citep{Komatsu:2015nkb, Komatsu:2012zh} --- so
\(\rho \propto H\) is doubly motivated, by the QCD contact term
\emph{and} by horizon thermodynamics, a convergence rather than a
coincidence. The QCD-topological-sector program remains active and is
being tested against current cosmological data
\citep{VanWaerbeke:2025shm}.

The contact term enters only in §\ref{sec-4}, and it is worth separating
at the outset what does not depend on it. The transmutation calculation,
the mirror-unification band and the glueball relic analysis take as
input only the value of \(\mu\), standard gauge-theory running, and
lattice pure-glue relations; none of them uses the contact term. Should
the Urban--Zhitnitsky mechanism fail, what remains is a hidden \(SU(3)\)
sector confining at 30--130 MeV with no new continuous parameters, whose
glueballs are a viable dark-matter candidate with a velocity-independent
self-interaction; a negative result --- that no perturbative order can
discriminate the two candidate coefficients (§\ref{sec-3-3}); and a
cosmology-free lattice test that decides the identification either way
(§\ref{sec-4-3}). What would be lost is the \emph{dark-energy
interpretation} of that sector, not the sector itself: the contact term
is the one conjectural load here, and it does not carry the rest.

The \(H\)-linear dark-energy phenomenology that motivates the scale ---
the equation of state, the growth gate and the modified-growth
signatures --- is developed in a companion cosmology manuscript, which
identifies the origin of \(\mu\) as its deepest open problem. Nothing
below depends on that development.

Each claim below is tagged by provenance: which quantities are
\emph{derived} (following from stated inputs by a reproducible
calculation), which are \emph{matched} to data (a computed number
compared against an observed or required value), which are
...
(\texttt{src/\allowbreak mu\_\allowbreak transmutation\_\allowbreak check.\allowbreak py}). The \(10^{60}\) tuning in
...
\subsection{Sensitivity of the transmutation map}\label{sec-2-2}
...
(\texttt{src/\allowbreak mu\_\allowbreak transmutation\_\allowbreak check.\allowbreak py}):
...
(\texttt{src/\allowbreak mu\_\allowbreak transmutation\_\allowbreak steps.\allowbreak py}) supersedes the early one-loop
...
Two conclusions follow. (i) Mirror unification with \(N_f = 6\)
...
from the UZ-corrected one (\(\Lambda_h = 6^{1/3}\mu\), §\ref{sec-4}).
The band brackets \emph{both} targets, so no alignment claim can be
drawn from it.

\subsection{The three-loop result: the band does not
tighten}\label{sec-3-3}

The scheme-fixed three-loop (and four-loop) running was computed as
Tier-1 item \#2 (\texttt{src/\allowbreak mirror\_\allowbreak qcd\_\allowbreak 3loop.\allowbreak py}), with the explicit
...
value to 28.5 vs 52 MeV (Figure 1).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/fig1_mirror_band.png}}
...
the transmutation route this is the single load-bearing assumption: if
the UZ term does not survive, the route delivers a natural scale with no
mechanism for the law.

\subsection{The structural argument: the contact term as a candidate
identity for the nonlocal degrees of freedom}\label{sec-4-2}

There is nonetheless a structural reason to take the mechanism
seriously, and it does not depend on accepting the contact term. A
volume-law horizon entropy \emph{cannot} be the entanglement entropy of
local quantum-field degrees of freedom: the Casini/Bekenstein bound
gives \(S_{\rm vol}/S_{\rm Bek} = 1/H_0 R\), so a volume-law count
necessarily forces \textbf{nonlocal horizon degrees of freedom}.
Identifying them is the central open structural problem for any
construction of this kind. The Veneziano-ghost/topological sector is
...
nonlocal character the entropy count demands. It also \emph{evades}
(does not violate) the Casini bound applied at all times, which excludes
local processes from producing the horizon count: the contact-term
vacuum energy is not local entanglement by construction. The
transmutation proposal is thus not only a candidate origin for \(\mu\);
it is a candidate identity for those nonlocal degrees of freedom. This
is an alignment of structures, not a derivation, and the mechanism
...
coverage. What makes the contact term \emph{gateable} remains an open
question --- a covariant formulation of the gate --- unchanged by this
proposal.
...
sector's cosmology \citep{Forestell:2017wov, Carenza:2023eua}. Neither
the candidate nor the cannibal relic mechanism is new here. What is
specific to this construction is that the confinement scale is not a
free parameter: it is the same \(\Lambda_h\) fixed in §\ref{sec-3} by
transmutation and, through the contact-term coefficient, tied to the
dark-energy amplitude. The dark-matter mass and the dark-energy scale
are then not independent inputs but one number in two roles,
\(m_G = 6\Lambda_h = 6^{4/3}\mu\) --- a relation that is
lattice-testable (§\ref{sec-4-3}) and that fails if either side moves.
Pure glue has no renormalizable portal to the Standard Model, so the
operative relic regime is the \textbf{decoupled cannibal} one
...
calculation (\texttt{src/\allowbreak glueball\_\allowbreak simp\_\allowbreak relic.\allowbreak py}: separate-entropy
...
\subsection{A mass miracle, not an abundance miracle}\label{sec-5-2}
...
This proposal \textbf{trades ECCG-ADM's \emph{explained}
\(\Omega_{\rm DM}/\Omega_b \approx 5\) for a \(Br \sim 2\times10^{-10}\)
reheating selection} --- one dial for another. The new dial is arguably
more natural (a tiny branching to a portal-less sector is generic; the
\(\times1.11\) band within it is not), but the ``why comparable to
baryons'' coincidence returns unexplained. The SIMP miracle here is a
\textbf{mass miracle} --- \(m_G\) lands where \(3\to2\) works with a
natural coupling, tied to the dark-energy scale --- \textbf{not an
abundance miracle}.

\subsection{A theorem-level observation}\label{sec-5-3}

A single-source constraint (``\(\eta_B\)-calculable XOR \(m_X\)-sharp'')
holds for \emph{clock-sourced} dark matter --- a construction can
deliver a calculable \(\eta_B\) or a sharp \(m_X\), but not both ---
with the premise that both yields ride the clock's \(\mu = H/2\pi\).
Glueball dark matter is \emph{thermally} sourced, so the premise fails,
and this branch achieves \textbf{both} a derived \(\eta_B\) (through the
Candidate-C clock channel) and a predicted DM mass --- the first branch
of the framework to do so. The theorem's content is conserved, relocated
...
(\texttt{src/\allowbreak glueball\_\allowbreak simp\_\allowbreak relic.\allowbreak py}):
...
(\texttt{src/\allowbreak glueball\_\allowbreak lrd\_\allowbreak seeding.\allowbreak py}: \(\sigma(M,z)\) from the linear
...
\textbf{Error budget.} The seed abundance traverses \(\sim 6\) orders of
magnitude between the DM-only and \(\times30\)-boosted brackets; the
boost factor, the collapse-time calibration \(C \approx 0.75\), the
concentration parameters \(\bar c\) and \(\sigma_{\ln c}\), and the 2\%
seed fraction are each \(O(1)\)-uncertain, and the abundance responds
\emph{exponentially} through the concentration tail (which is trusted
only to \(\sim 4\sigma\), beyond which it is not simulation-calibrated).
The ``\(\Lambda_h = 52\) lands on target'' statement is therefore a
\textbf{viability statement, not a prediction}: the claim is that
\emph{within the plausible baryon-boost bracket,
\(\Lambda_h \approx 52\) MeV crosses \(n_{\rm LRD}\) while 105 cannot
and 30 over-seeds}.

\subsection{The pincer: one live bound and one conditional bracket (an
...
mechanism requires. This is a convergence of \emph{constraints} (one of
them conditional on an unresolved astrophysical question), not a
measurement of \(\Lambda_h\).** The lattice
\(T_c/\Lambda_{\overline{\rm MS}}\) calculation (§\ref{sec-3-4}) would
supply a genuinely independent, non-astrophysical probe --- either
converging on one number, or failing to and falsifying the transmutation
identification (Figure 2).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/fig2_pincer.png}}
...
\item
  ...
  then in this framework: \(\Lambda_h = 105\) MeV is dead as the DM
  scale and the surviving window is \(\Lambda_h \approx 30\)--52 MeV
  with baryon-boosted collapse. If LRDs are resolved astrophysically
  (the current mainstream expectation), this channel goes quiet and
  nothing here changes. \textbf{The trigger condition is not currently
  met and may never be}; the falsifier has not been appended to the
...
One robustness statement belongs alongside the falsifier list rather
than in it. Because the hidden sector is cold (\(\xi \approx 0.005\))
and the glueball heavy (\(m_G \approx 312\) MeV), cannibal freeze-out
occurs at \(T_{\rm vis} \approx 3\) GeV, and both the free-streaming and
cannibal-horizon cutoffs lie below \(10^{-10}\,M_\odot\) --- some
eighteen orders of magnitude beneath the \(\sim 10^8\,M_\odot\)
subhaloes detected by gravitational imaging (e.g.~JVAS B1938+666). The
sector is CDM-like on all observable scales, as a \emph{derived}
consequence of its coldness rather than an assumption; substructure
lensing does not constrain it, and the SIDM channel (\(\sigma/m = 0.09\)
cm²/g) sits well below the \(\sim 1\) cm²/g scale at which subhalo
evaporation becomes diagnostic in lensing --- the binding
self-interaction constraint remains the cluster bound of §\ref{sec-6}.
...
\textbf{conjectural and contested} --- the one candidate compatible with
nonlocal horizon degrees of freedom; lattice discharge test specified \\
...
\item
  ...
  We have argued it is uniquely aligned with the nonlocal horizon
  degrees of freedom that a volume-law count independently forces, and
  that it evades the Casini no-go, and we have specified a
  cosmology-free lattice test --- but until that test (or an equivalent)
  is passed, the mechanism remains a conjecture and the route is
  incomplete.
\item
  ...
  point marking the failure of perturbative control.
\item
...
\item
  ...
  to the framework (the high-scale \(SU(3)_H\) at \(6\times10^{12}\)
  GeV, the dark-force \(V/\varphi\) sector, and now mirror QCD at
  \(\sim 50\) MeV) --- a real cost against the economy claim, unless the
  mirror sector can be related to existing structure. Against this
  stands the strongest form of the economy accounting:
  ...
  economy, with the \(Br\) dial and the UZ conjecture as the residuals.
\item
  \textbf{Mutual exclusivity with the visible-QCD origin.} The
  transmutation origin and the alternative visible-QCD hypothesis
  (\(b_{\rm QCD} = 0.2062\)), developed in the companion cosmology
  manuscript, are mutually exclusive and distinguishable (§\ref{sec-7},
  item 6); only one can be right, and current growth data
  (\(\mu_0 = 0.04 \pm 0.22\)) do not yet decide.
\item
...
new continuous input at all. The score is: a zero-parameter decade-level
success for the scale (band 28--133 MeV, irreducibly \(O(1)\)-wide
perturbatively, with a specified lattice closer); one contested
load-bearing conjecture for the law (UZ), aligned with the nonlocal
horizon degrees of freedom that a volume-law count independently
requires and equipped with a cosmology-free lattice discharge test; a
...
\texttt{src/\allowbreak }; a tagged release is archived at Zenodo, DOI
10.5281/zenodo.21525532):

\begin{itemize}
\tightlist
\item
  \texttt{src/\allowbreak mu\_\allowbreak transmutation\_\allowbreak check.\allowbreak py} --- §\ref{sec-2} coupling
  check and sensitivity; §\ref{sec-3-1} one-loop \(N_f\) scan;
  §\ref{sec-4} UZ coefficient target.
\item
  \texttt{src/\allowbreak mu\_\allowbreak transmutation\_\allowbreak steps.\allowbreak py} --- §\ref{sec-3-2} two-loop +
  threshold band; the \(b\)-freed sum rule and measurability forecast;
  §\ref{sec-5-4} subcomponent \(\xi\) boundaries and the \(m_X\)-shift
  window.
\item
  \texttt{src/\allowbreak mirror\_\allowbreak qcd\_\allowbreak 3loop.\allowbreak py} --- §\ref{sec-3-3} three- and
  four-loop running, the spurious IR fixed point, and the 28--133 MeV
  band.
\item
  \texttt{src/\allowbreak glueball\_\allowbreak simp\_\allowbreak relic.\allowbreak py} --- §\ref{sec-5}
  decoupled-cannibal freeze-out, \(\alpha_{\rm eff}\) and \(\xi\)/\(Br\)
  bands, \(\Delta N_{\rm eff}\), and the §\ref{sec-6-1} \(\sigma/m\)
  ladder.
\item
  \texttt{src/\allowbreak glueball\_\allowbreak lrd\_\allowbreak seeding.\allowbreak py} --- §\ref{sec-6-2} gravothermal
...
researcher, declares no competing interests. Ethics approval is not
applicable: this work is theoretical and involved no human participants,
no human data or tissue, and no animal subjects.
