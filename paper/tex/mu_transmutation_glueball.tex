...
It is worth stating at the outset what survives if the contact term does
not. The transmutation calculation, the mirror-unification band and the
glueball relic analysis take as input only the value of \(\mu\),
standard gauge-theory running, and lattice pure-glue relations; none of
them uses the contact term. Should the Urban--Zhitnitsky mechanism fail,
what remains is a hidden \(SU(3)\) sector confining at 30--130 MeV with
no new continuous parameters, whose glueballs are a viable dark-matter
candidate with a velocity-independent self-interaction; a negative
result --- that no perturbative order can discriminate the two candidate
coefficients (§\ref{sec-3-3}); and a cosmology-free lattice test that
decides the identification either way (§\ref{sec-4-3}). What would be
lost is the \emph{dark-energy interpretation} of that sector, not the
sector itself. We flag this because the contact term is the one
...
to and falsifying the transmutation identification (Figure 2).
